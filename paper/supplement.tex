...
\label{tab:s1}
\input{tables/tab_cas_seed_effects}
\end{table}
...
\label{tab:s2}
\input{tables/tab_lrd_confirmation}
\end{table}
...
\label{tab:s3}
\input{tables/tab_sen12_conditions}
\end{table}
...
\label{tab:s4}
\input{tables/tab_reviewer_remediation}
\end{table}

\begin{table}[htbp]
\centering
\caption{Two-order sensitivity envelopes over release-supported RGBN and
conservative native-order BGRN sensitivity. Envelopes span mapping-specific
descriptive means without assigning equal plausibility; they are not confidence
intervals.}
\label{tab:s5}
\input{tables/tab_semantic_identifiability}
\end{table}
